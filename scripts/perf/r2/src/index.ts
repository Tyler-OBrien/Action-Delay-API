...
const handler = {
	async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		if (url.pathname.startsWith('/uploadrandom10kb')) {
			const buffer = new Uint8Array(10_000);
			crypto.getRandomValues(buffer);
			key = key.replace('uploadrandom10kb/', '');
			var uploadAndDownloadStart = performance.now();
			try {
				const putResult = await (env[getBucketLocation.region] as R2Bucket).put(key, buffer);
				if (!putResult) {
					return new Response('Error R2 PUT: Failure Uploading', {
						status: 500,
						headers: {
							region: getBucketLocation.region,
							routing: getBucketLocation.routing,
							'x-adp-dur': (performance.now() - uploadAndDownloadStart).toString(),
						},
					});
				}
				...
				return new Response(`Error R2 PUT: ${exception}`, {
					status: 500,
					headers: {
						'x-adp-dur': (performance.now() - uploadAndDownloadStart).toString(),
						region: getBucketLocation.region,
					...
					try {
						if (env.AE)
							env.AE.writeDataPoint({
								blobs: [getBucketLocation.region, getBucketLocation.routing, req.cf?.colo],
								doubles: [cacheDur],
								indexes: [key + '-cache'],
							});
					} catch (exception) {
		...
		try {
			...
			console.log(exception);
			return new Response(`Error R2 GET: ${exception}`, {
				status: 500,
				headers: {
		...
		if (object === null) {
			...
			if (isCachingEnabled) newResp.headers.set('x-cache-dur', cacheDur.toString());
		...
		if (isCachingEnabled) headers.set('x-cache-dur', cacheDur.toString());
		...
		if (isCachingEnabled) ctx.waitUntil(cache.put(req, newResp.clone()));
		return newResp;
	},
};
...
const getRegion = function (iata: regions, continent: string | undefined): GetRegionResult {
	...
	switch (continent) {
		...
		case 'AN':
			return new GetRegionResult('enam', 'fallback-continent');
		case 'OC':
			return new GetRegionResult('oc', 'fallback-continent');
		case 'AS':
