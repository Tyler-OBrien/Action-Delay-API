...
const handler = {
  async fetch(request: Request, env: Env) {
    ...
    if (
      request.headers.get("Authorization") !== env.APIKEY &&
      url.searchParams.get("header_Authorization") !== env.APIKEY
    ) {
    ...
    for await (const event of streamAsyncIterator(events)) {
      ...
      try {
        ...
        parsedEvent["eventDelay"] =
          timeStart.getTime() - parsedTimestamp.getTime();
      } catch (error: any) {
        console.log(
          `Error parsing EndEdgeTimestamp: ${error}, ${error?.cause}`
        );
      }
	...
	*/
    ...
    while (true) {
      ...
      if (attempt == 0) backoffMs = 1;
      attempt += 1;
      if (attempt >= maxAttempts) {
        // Reached max attempts, so don't retry.
        console.log(`We tried ${attempt} times, but failed each time`)
        throw error;
      }
...
class Batcher implements DurableObject {
  ...
  async alarm() {
    ...
    if (this.queue.length == 0) {
      console.log(
        `[${this.ID}] No items to push, returning. We thought there would be ${currentCount}`
      );
      return;
    }
    ...
    console.log(
      `[${this.ID}] Alarm! Pushing ${this.queue.length} items.. we thought there would be ${currentCount}`
    );
...
handleHTTPEvents(env: Env, httpEvents: any): Promise<void> {
  ...
  httpEvents.forEach((event: any) => {
    ...
    if (
      event.UpperTierColoID != 0 &&
      event.UpperTierColoID.length != 0 &&
      !uniqueUpperTierColos[event.UpperTierColoID]
    )
      uniqueUpperTierColos[event.UpperTierColoID] = true;

    if (
      event.SmartRouteColoID != 0 &&
      event.SmartRouteColoID.length != 0 &&
      !uniqueSmartRoutingColos[event.SmartRouteColoID]
    )
  ...
  try {
    ...
    try {
      ...
      var db2result = await env.DB2.batch(
        uniqueEventsArray.map((event: any) =>
          preparedStmt2.bind(
            event.EdgeColoID,
            parseInt(event.ResponseHeaders.metal)
          )
        )
      );
      console.log(
        `DB2 SQLITE  Execute:  count: ${db2result.reduce(
          (accumulator, item) => accumulator + item.meta.changes,
          0
        )}, dbtime: ${db2result.reduce(
          (accumulator, item) => accumulator + item.meta.duration,
          0
        )}ms, actual time: ${Math.round(Date.now() - requestStartTime)}ms`
      );
      var requestStartTime = Date.now();
      var preparedStmt2colos = env.DB2.prepare(sqlite2);
      var db2resultcolos = await env.DB2.batch(
        uniqueEventsArray
          .filter(
            (event: any) => event.EdgeColoCode && event.EdgeColoCode.length != 0
          )
          .map((event: any) =>
            preparedStmt2colos.bind(
              event.EdgeColoID,
              event.EdgeColoCode,
              uniqueSmartRoutingColos[event.EdgeColoID] ? 1 : 0,
              uniqueUpperTierColos[event.EdgeColoID] ? 1 : 0
            )
          )
      );
      console.log(
        `DB2 SQLITE Colos  Execute:  count: ${db2resultcolos.reduce(
          (accumulator, item) => accumulator + item.meta.changes,
          0
        )}, dbtime: ${db2resultcolos.reduce(
          (accumulator, item) => accumulator + item.meta.duration,
          0
        )}ms, actual time: ${Math.round(Date.now() - requestStartTime)}ms`
      );
    ...
    try {
      ...
      var dbresult = await env.DB.batch(
        uniqueEventsArray.map((event: any) =>
          preparedStmt.bind(
            event.EdgeColoID,
            parseInt(event.ResponseHeaders.metal)
      ...
      var dbresultcolos = await env.DB.batch(
        uniqueEventsArray
          .filter(
            (event: any) => event.EdgeColoCode && event.EdgeColoCode.length != 0
          )
          .map((event: any) =>
            preparedStmtcolos.bind(
              event.EdgeColoID,
              event.EdgeColoCode,
              uniqueSmartRoutingColos[event.EdgeColoID] ? 1 : 0,
              uniqueUpperTierColos[event.EdgeColoID] ? 1 : 0
      ...
      console.log(
        `DB SQLITE Colos  Execute:  count: ${dbresultcolos.reduce(
          (accumulator, item) => accumulator + item.meta.changes,
          0
        )}, dbtime: ${dbresultcolos.reduce(
          (accumulator, item) => accumulator + item.meta.duration,
          0
        )}ms, actual time: ${Math.round(Date.now() - requestStartTime)}ms`
      );
