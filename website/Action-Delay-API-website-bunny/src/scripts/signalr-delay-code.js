...
async function start() {
  try {
    ...
    await connection.invoke("SubscribeToType", "BunnyDelay");
  } catch (err) {
    console.log(err);
...
setInterval(async () => {
  try {
    const getJobs = await (
      await fetch(
        "https://api.bunny.chaika.me/v2/jobs/type/BunnyDelay"
      )
    ).json();
    ...
    console.error(exception);
    console.log(`Error in job refresh, are you connected to the internet?`);
  }
}, 30_000);
...
setInterval(async () => {
  try {
    const getAnalytics = await (
      await fetch(
        "https://api.bunny.chaika.me/v1/quick/QuickAnalytics/type/BunnyDelay"
      )
    ).json();
    ...
    console.error(exception);
    console.log(
      `Error in job analytics refresh, are you connected to the internet?`
    );
  }
}, 120_000);
