...
setInterval(async () => {
  try {
    const getJobs = await (
      await fetch(
        `https://delay.cloudflare.chaika.me/v2/jobs/${globalThis.jobData.internalJobName}`
      )
    ).json();
...
setInterval(async () => {
  try {
    const getAnalytics = await (
      await fetch(
        `https://delay.cloudflare.chaika.me/v1/quick/QuickAnalytics/${globalThis.jobData.internalJobName}`
      )
    ).json();
