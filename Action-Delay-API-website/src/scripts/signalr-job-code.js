...
.withUrl("https://delay.cloudflare.chaika.me/v2/jobs/signalr", {         transport: signalR.HttpTransportType.WebSockets, skipNegotiation: true  })
...
var getCurrentData = globalThis.jobData.find(job => job.internalJobName == data.JobName);
if (getCurrentData) {
// new state is now old!
    ...
    getCurrentData.currentRunTime = new Date(new Date().getTime() - data.RunLength);
...
setInterval(async () => {
    ...
    globalThis.jobData = getJobs.data.filter(job => job.internalJobName != "workertesting");
    }
    catch (exception) {
        console.error(exception);
...
setInterval(async () => {
    try {
        ...
        globalThis.jobStats = getAnalytics;
            ...
            console.error(exception);
            console.log(`Error in job analytics refresh, are you interwebular connected?`)
        }

}, 120_000)

async function handleJobUpdateData(internalJobNameUpdate) {
var getCurrentData = globalThis.jobData.find(job => job.internalJobName == internalJobNameUpdate);
var tryGetAnalytics = globalThis.jobStats.filter(job => job.job_name);
let lastUpdatedTxt = "";
let delayTxt = "";
let delayTxtUnit = "";
if (!getCurrentData) {
    console.log(`Couldn't get current data for ${internalJobNameUpdate}, aborting?`)
    return;
}
let pending = document.getElementById('pending' + getCurrentData.internalJobName);
...
lastUpdatedTxt = formatTime(new Date() - new Date(new Date(runTimeToUse).getTime() + runLengthToUse)) + " ago";
if (job.internalJobName === "cron") {
	noLastUpdate = true;
}
if (pending.textContent == '' && (job.currentRunStatus == 'Pending' || job.currentRunStatus == 'Undeployed' )) {

    if (job.currentRunStatus == 'Pending')
...
if (noLastUpdate == false) {
    document.getElementById(`lastUpdate${job.internalJobName}`).innerText = `Last Update: ${lastUpdatedTxt}`
}

    document.getElementById(`delayNumber${job.internalJobName}`).innerText = `${delayTxt}`
    document.getElementById(`delayUnit${job.internalJobName}`).innerText = `${delayTxtUnit}`
}
